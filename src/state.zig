...
const RadioState = struct {
    ...
    receiver: ?radio.RadioReceiver = null,
...
pub fn main() !void {
    ...
    rg.guiSetStyle(rg.DEFAULT, rg.BASE_COLOR_NORMAL, 0x2b2d30ff);
    rg.guiSetStyle(rg.DEFAULT, rg.TEXT_COLOR_NORMAL, 0xffffffff);
    rg.guiSetStyle(rg.DEFAULT, rg.BASE_COLOR_FOCUSED, 0x5cb3ccff);
    rg.guiSetStyle(rg.DEFAULT, rg.BORDER_COLOR_FOCUSED, 0x84d7f2ff);
    rg.guiSetStyle(rg.DEFAULT, rg.TEXT_SIZE, 14);

    var state = RadioState{};

    // Initialize some demo presets
    state.presets[0] = .{ .frequency = 88.5, .mode = .FM, .band = .FM_US, .name = "Classic Rock FM".*, .active = true };
    state.presets[1] = .{ .frequency = 94.7, .mode = .FM, .band = .FM_US, .name = "Jazz FM".*, .active = true };
    state.presets[2] = .{ .frequency = 101.1, .mode = .FM, .band = .FM_US, .name = "News Radio".*, .active = true };
    state.presets[3] = .{ .frequency = 650.0, .mode = .AM, .band = .AM_MW, .name = "Sports AM".*, .active = true };
...
fn drawTitleBar(state: *RadioState, w: f32) void {
    ...
    // Title
    rl.drawText("AM/FM Radio", 10, 10, 20, rl.WHITE);

    // Power button
    const power_rect = rl.Rectangle{ .x = w - 50, .y = 5, .width = 40, .height = 30 };
    const power_color = if (state.is_powered) rl.Color{ .r = 92, .g = 179, .b = 204, .a = 255 } else rl.GRAY;
    _ = power_color; // autofix

    if (rg.guiButton(power_rect, if (state.is_powered) "ON" else "OFF") == 1) {
    ...
    if (state.is_powered) {
        if (state.stereo_detected) {
            rl.drawText("STEREO", @intFromFloat(w - 150), 12, 16, rl.GREEN);
        } else if (state.mode == .FM) {
            rl.drawText("MONO", @intFromFloat(w - 150), 12, 16, rl.YELLOW);
        }

        if (state.is_recording) {
            rl.drawCircle(@intFromFloat(w - 200), 20, 5, rl.RED);
            rl.drawText("REC", @intFromFloat(w - 190), 12, 16, rl.RED);
        }
    }
}
...
fn drawFrequencyDisplay(state: *RadioState, w: f32) void {
    ...
    if (!state.is_powered) {
        rl.drawText("---.--", @intFromFloat(w / 2 - 80), y_base + 20, 48, rl.DARKGRAY);
        return;
    }
    ...
    if (rl.checkCollisionPointRec(rl.getMousePosition(), freq_rect) and rl.isMouseButtonPressed(.MOUSE_BUTTON_LEFT)) {
    ...
    if (state.freq_editing) {
        ...
        rl.drawText(freq_str, @intFromFloat(w / 2) - @divTrunc(text_width, 2), y_base + 20, 48, rl.WHITE);
    }
}
...
fn drawSignalMeter(state: *RadioState, w: f32) void {
    ...
    rl.drawText("Signal", 15, y_base, 14, rl.LIGHTGRAY);
    ...
    // Signal level
    var signal_color = rl.RED;
    if (signal_pct > 0.3) signal_color = rl.YELLOW;
    if (signal_pct > 0.6) signal_color = rl.GREEN;
...
fn drawModeAndBandSelection(state: *RadioState, w: f32) void {
    ...
    // Mode selection
    rl.drawText("Mode", 15, y_base, 14, rl.LIGHTGRAY);
    const modes = [_][]const u8{ "FM", "AM", "NFM", "USB", "LSB" };
    _ = modes; // autofix
    var mode_index: c_int = @intFromEnum(state.mode);

    const mode_rect = rl.Rectangle{ .x = 60, .y = @floatFromInt(y_base - 2), .width = 160, .height = 25 };
    _ = rg.guiComboBox(mode_rect, "FM;AM;NFM;USB;LSB", &mode_index);
    if (state.is_powered and mode_index != @intFromEnum(state.mode)) {
        state.mode = @enumFromInt(@as(u8, @intCast(mode_index)));
    }

    // Band selection
    rl.drawText("Band", 240, y_base, 14, rl.LIGHTGRAY);
    ...
    _ = rg.guiComboBox(band_rect, "FM US;FM Japan;Weather;AM MW;Free", &band_index);
    if (state.is_powered and band_index != @intFromEnum(state.band)) {
...
fn drawTuningControls(state: *RadioState, w: f32) void {
    ...
    if (state.is_powered) {
        ...
        if (rg.guiButton(tune_down_fine, "<<") == 1) {
            state.frequency -= limits.step;
            if (state.frequency < limits.min) state.frequency = limits.max;
            state.is_scanning = false;
        }

        if (rg.guiButton(tune_down, "<") == 1) {
            state.frequency -= limits.step * 10;
            if (state.frequency < limits.min) state.frequency = limits.max;
            state.is_scanning = false;
        }

        if (rg.guiButton(scan_down, "SCAN-") == 1) {
            state.is_scanning = true;
            state.frequency -= limits.step;
        }

        if (rg.guiButton(scan_up, "SCAN+") == 1) {
            state.is_scanning = true;
            state.frequency += limits.step;
        }

        if (rg.guiButton(tune_up, ">") == 1) {
            state.frequency += limits.step * 10;
            if (state.frequency > limits.max) state.frequency = limits.min;
            state.is_scanning = false;
        }

        if (rg.guiButton(tune_up_fine, ">>") == 1) {
            state.frequency += limits.step;
            if (state.frequency > limits.max) state.frequency = limits.min;
            state.is_scanning = false;
        }
        ...
        // Disabled state
        rg.guiSetState(rg.STATE_DISABLED);
        _ = rg.guiButton(tune_down_fine, "<<");
        _ = rg.guiButton(tune_down, "<");
        _ = rg.guiButton(scan_down, "SCAN-");
        _ = rg.guiButton(scan_up, "SCAN+");
        _ = rg.guiButton(tune_up, ">");
        _ = rg.guiButton(tune_up_fine, ">>");
        rg.guiSetState(rg.STATE_NORMAL);
    }

    // Scan indicator
    if (state.is_scanning) {
        rl.drawText("SCANNING...", @intFromFloat(w / 2 - 40), y_base + 40, 14, rl.YELLOW);
    }
}
...
fn drawVolumeAndSquelch(state: *RadioState, w: f32) void {
    ...
    // Volume
    rl.drawText("Volume", 15, y_base, 14, rl.LIGHTGRAY);
    const vol_rect = rl.Rectangle{ .x = 70, .y = @floatFromInt(y_base - 2), .width = w - 140, .height = 20 };
    state.volume = rg.guiSliderBar(vol_rect, null, null, state.volume, 0.0, 1.0);

    var vol_buf: [16]u8 = undefined;
    const vol_str = std.fmt.bufPrintZ(&vol_buf, "{d:.0}%", .{state.volume * 100}) catch "0%";
    rl.drawText(vol_str, @intFromFloat(w - 50), y_base, 14, rl.LIGHTGRAY);

    // Squelch
    rl.drawText("Squelch", 15, y_base + 30, 14, rl.LIGHTGRAY);
    const sq_rect = rl.Rectangle{ .x = 70, .y = @floatFromInt(y_base + 28), .width = w - 140, .height = 20 };
    state.squelch = rg.guiSliderBar(sq_rect, null, null, state.squelch, 0.0, 1.0);

    var sq_buf: [16]u8 = undefined;
    const sq_str = std.fmt.bufPrintZ(&sq_buf, "{d:.0}%", .{state.squelch * 100}) catch "0%";
    rl.drawText(sq_str, @intFromFloat(w - 50), y_base + 30, 14, rl.LIGHTGRAY);

    // Gain (for SDR control)
    rl.drawText("Gain", 15, y_base + 60, 14, rl.LIGHTGRAY);
    const gain_rect = rl.Rectangle{ .x = 70, .y = @floatFromInt(y_base + 58), .width = w - 140, .height = 20 };
    state.gain = rg.guiSliderBar(gain_rect, null, null, state.gain, 0.0, 50.0);

    var gain_buf: [16]u8 = undefined;
    const gain_str = std.fmt.bufPrintZ(&gain_buf, "{d:.0} dB", .{state.gain}) catch "0 dB";
    rl.drawText(gain_str, @intFromFloat(w - 50), y_base + 60, 14, rl.LIGHTGRAY);
}

fn drawPresets(state: *RadioState, w: f32) void {
    const y_base = 370;

    rl.drawText("Presets", 15, y_base, 14, rl.LIGHTGRAY);
    rl.drawLine(10, y_base + 20, @intFromFloat(w - 10), y_base + 20, rl.DARKGRAY);
    ...
    while (i < 12) : (i += 1) {
        ...
        if (state.presets[i].active) {
            ...
            const label = std.fmt.bufPrintZ(&label_buf, "{d:.1}\n{s}", .{ state.presets[i].frequency, state.presets[i].name[0..std.mem.len(@as([*:0]u8, @ptrCast(&state.presets[i].name)))] }) catch "Empty";
            ...
            if (rg.guiButton(preset_rect, label) == 1 and state.is_powered) {
                // Load preset
            ...
            // Empty preset slot
            if (rg.guiButton(preset_rect, "+") == 1 and state.is_powered) {
...
fn drawBottomControls(state: *RadioState, w: f32) void {
    ...
    if (state.is_powered) {
        ...
        if (rg.guiButton(rec_rect, rec_label) == 1) {
            state.is_recording = !state.is_recording;
        }
    } else {
        rg.guiSetState(rg.STATE_DISABLED);
        _ = rg.guiButton(rec_rect, "RECORD");
        rg.guiSetState(rg.STATE_NORMAL);
    }
    ...
    if (state.is_powered and state.is_scanning) {
        if (rg.guiButton(stop_rect, "STOP SCAN") == 1) {
            state.is_scanning = false;
        }
    } else {
        rg.guiSetState(rg.STATE_DISABLED);
        _ = rg.guiButton(stop_rect, "STOP SCAN");
        rg.guiSetState(rg.STATE_NORMAL);
    }

    // Settings button (placeholder)
    const settings_rect = rl.Rectangle{ .x = 20 + button_width * 2, .y = @floatFromInt(y_base), .width = button_width, .height = 40 };
    if (rg.guiButton(settings_rect, "SETTINGS") == 1) {
    ...
    if (state.is_powered) {
        ...
        rl.drawText(status, 10, y_base + 50, 12, rl.LIGHTGRAY);
    } else {
        rl.drawText("RTL-SDR: Disconnected", 10, y_base + 50, 12, rl.DARKGRAY);
    }
}
